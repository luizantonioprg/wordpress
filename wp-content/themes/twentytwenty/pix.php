...
<?php
if(isset($_POST['pagar_pix'])):
  if(empty($_POST['email']) || empty($_POST['first_name']) || empty($_POST['last_name']) || empty($_POST['cpf']) || empty($_POST['cep']) || empty($_POST['rua']) || empty($_POST['bairro']) || empty($_POST['cidade']) || empty($_POST['estado'])):
  ...
  elseif(strlen($_POST['estado']) > 50):
    ...
    MercadoPago\SDK::setAccessToken("XXXXXXXXX");
    ...
    $payment->payer = array(
        "email" => "test_user_80254437@testuser.com",
        "first_name" => "Test",
        "last_name" => "User",
...
$_SESSION['LIMPARTELA'] = true;
    $source = $payment->point_of_interaction->transaction_data->qr_code_base64;
    $codigo = $payment->point_of_interaction->transaction_data->qr_code;
   // setcookie("LIMPARTELA",true);
    echo "<img src='data:image/png;base64, ".$source."' width=200px height=200px>";
    //echo "<h3>Código de Pagamento:$codigo</h3>";
...
$resp = $wpdb->query($wpdb->prepare($sql, array($_POST['email'],$_POST['cep'],$_POST['rua'],$_POST['bairro'],$_POST['cpf'],$_POST['complemento'],$_POST['cidade'],$_POST['estado'],'PENDENTE','PIX',$_POST['carrinho'],$_POST['total_carrinho'],$_POST['id_cliente'])));
  endif;
endif;
...
<form method='post' action='' id="formulario">
        ...
        <input type='hidden' name='total_carrinho' value='<?php echo $_SESSION['valor_pix']?>'>
        <label>Email<br>
        ...
        <button type='submit' name='pagar_pix'>PAGAR</button>
  </form>
<?php endif; ?>
